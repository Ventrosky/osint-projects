...
for json_file in file_list:
    ...
    with open(json_file,"rb") as fd:
        try:
            ...
            if scan_result.get('linkedSites', None) is not None:
                
                edges.extend(scan_result['linkedSites'])
            
            if scan_result.get('relatedOnionDomains', None) is not None:
                
                edges.extend(scan_result['relatedOnionDomains'])
            
            if scan_result.get('relatedOnionServices', None) is not None:
            ...
            if scan_result.get('ipAddresses', None) is not None:
